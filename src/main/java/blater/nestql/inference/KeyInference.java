...
public final class KeyInference {
  ...
  private static DatabaseStructure structure(
    ...
    if (!forceRefresh && cached.isPresent() && cached.get().version() == GRAPH_VERSION
      ...
      try {
        ...
        DatabaseStructure structure = deserialize(cached.get().payload());
        if (referencedRelations.isEmpty()
            || new DatabaseStructureInferrer().matches(
    ...
    DatabaseStructure inferred = new DatabaseStructureInferrer().infer(executor.connection());
    try {
  ...
  private static byte[] serialize(DatabaseStructure structure) throws IOException {
  ...
  private static DatabaseStructure deserialize(byte[] payload) throws IOException, ClassNotFoundException {
    try (ObjectInputStream input = new ObjectInputStream(new ByteArrayInputStream(payload))) {
      Object value = input.readObject();
      if (!(value instanceof DatabaseStructure structure)) {
        throw new IOException("Unexpected cached graph type");
      }
      return structure;
    }
  }
